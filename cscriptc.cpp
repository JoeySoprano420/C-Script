...
// Version information
// ============================================================================
constexpr const char* CSCRIPT_VERSION = "0.4.1.1";
...
static void check_exhaustiveness_or_die(const string& src,
    ...
    while (true) {
        ...
        std::regex caseRe(R"(CS_CASE\s*\(\s*([A-ZaZ_]\w*)\s*\))");
...
/*
==========================================================================
C-Script Language Reference (v0.4.0)
==========================================================================

# Overview

C-Script is a lightweight language that compiles to C, providing:
- Zero-cost abstractions with direct C compatibility
- Strong static analysis with exhaustiveness checking
- Profile-guided optimization for performance
- Pattern matching and softline functions

# Key Language Features

## 1. Enum Declarations with Exhaustiveness
/*
==========================================================================
C-Script Language Reference (v0.4.0)
==========================================================================
...
C-Script's `enum!` declarations allow specifying all possible values for a type.
The compiler verifies that all enum cases are handled in `switch` statements.
...
## 2. Softline Functions

Softline functions use `=>` for single-expression functions, with optional type annotations.
...
C-Script checks that all enum cases are handled in `switch` statements.
It reports errors for unhandled cases when `hardline` checks are enabled.

## 4. @unsafe Pragmas

The `@unsafe` pragma disables certain checks and allows unsafe operations.
...
C-Script includes automatic error correction that detects and fixes common syntax errors:

- Missing semicolons are automatically added.
- Mismatched braces are corrected.
- Identifier typos are fixed by suggesting the closest valid identifier.
- Invalid enum values are replaced with the first valid value.

# Usage

The error correction system is enabled by default. Use these options to control it:

  --strict         Disable automatic correction and fail on errors
  --verbose        Show details about corrections being applied

Example output:

Applying automatic error correction...
Applied 3 automatic corrections:
  Line 10: Added missing semicolon
  - Original: int x = 5
  - Corrected: int x = 5;
  Line 15: Removed extra closing brace
  - Original: }
  - Corrected:
  Line 20: Corrected identifier to closest match
  - Original: Colr
  - Corrected: Color
  ==========================================================================
  Line 25: Replaced invalid enum value with first valid value
  - Original: Color c = Colr::Blu;
  - Corrected: Color c = Color::Red;
  ==========================================================================
  # Limitations

  - The system may not always infer the correct intent, especially in complex scenarios
  - It is recommended to review the corrections and adjust the source code as needed
  - The system currently focuses on common syntax errors and may not cover all cases
  - Use the --strict option to disable corrections if precise control is required
  ==========================================================================

This feature helps maintain programming flow by reducing the compile-edit-compile
cycle, especially for trivial syntax errors.
*/

// ============================================================================
// ==========  PREDICTIVE PROGRAMMING & ADVANCED CORRECTION SYSTEM  ===========
// ============================================================================
...
namespace build_autofix {

    static bool is_msvc(const std::string& cc) {
        return (cc == "cl" || cc == "clang-cl");
    }
    ...
    static std::string light_repair_c(const std::string& src) {
        std::istringstream in(src);
        std::string line, out;
        out.reserve(src.size() + src.size() / 32);
        ...
        while (std::getline(in, line)) {
            std::string raw = line;
            std::string stripped = rtrim(line);
            size_t i = 0; while (i < stripped.size() && (stripped[i] == ' ' || stripped[i] == '\t')) ++i;
            stripped.erase(0, i);
            ...
            if (!stripped.empty() && stripped[0] == '#') {
                ...
                bool endsWithBrace = !stripped.empty() && (stripped.back() == '{' || stripped.back() == '}');
        ...
        while (openBr > closeBr) { out += "}\n"; ++closeBr; }
        return out;
    }

    static std::string inject_flag(const std::string& cmd, const std::string& flag) {
        if (cmd.find(flag) != std::string::npos) return cmd;
        std::string out = cmd; out.push_back(' '); out += flag; return out;
    }

    static std::string ensure_link_lib(const std::string& cmd, const std::string& libFlag) {
        return (cmd.find(libFlag) != std::string::npos) ? cmd : inject_flag(cmd, libFlag);
    }
    ...
    static int compile_attempt(const Config& cfg,
        ...
        if (mutKind == 1) {
            if (is_msvc(cc)) cmd = inject_flag(cmd, "/utf-8");
            else {
        ...
        if (cfg.verbose) std::cerr << "[auto-fix] " << note << "\nCC: " << cmd << "\n";
        int rc = run_cmd(cmd, false);
        if (!cfg.show_c) rm_file(cpath);
        return rc;
    }
    ...
    static int run_cmd_with_attempts(const Config& cfg,
        ...
        std::string asciiC = ascii_sanitize(c_src);
        if (compile_attempt(cfg, cc, asciiC, out, defineProfile, "Attempt 4: ASCII-sanitize generated C", 1) == 0) return 0;

        std::string repaired = light_repair_c(asciiC);
        if (compile_attempt(cfg, cc, repaired, out, defineProfile, "Attempt 5: semicolon/brace repair", 1) == 0) return 0;
...
static int build_once_autofix(const Config& cfg,
    ...
    if (cfg.verbose) {
        std::cerr << "Building (auto-fix enabled)...\n";
    }
    return build_autofix::run_cmd_with_attempts(cfg, cc, c_src, out, defineProfile);
}
