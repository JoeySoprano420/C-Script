...
static string softline_lower(const string& src,
    ...
    // 2) block fn header: fn name(args) -> ret {
    {
        ...
        while (search_from(s, pos, m, r)) {
            ...
            // Replace this buggy line inside softline_lower (block fn header):
            // if (instrument) hdr << "cs_prof_hit("\" << name << "\"); ";
        ...
        // With this fixed version:
        // Remove the erroneous line inside softline_lower (block fn header):
        // if (instrument) :  "cs_prof_hit(\"" << name << "\"); ";

        // The correct code is already present above:
        // if (instrument) hdr << "cs_prof_hit(\"" << name << "\"); ";

        // So, simply delete the buggy line below the block fn header loop.

        // --- FIXED SECTION ---
        // In softline_lower, block fn header section, remove the erroneous line:
        // if (instrument) :  "cs_prof_hit(\"" << name << "\"); ";
        // In softline_lower, block fn header section, remove the erroneous line:
        // if (instrument)  <<  "cs_prof_hit(\"" << name << "\"); ";
...
https://github.com/JoeySoprano420/C-Script/blob/main/Documentation.md
*/
